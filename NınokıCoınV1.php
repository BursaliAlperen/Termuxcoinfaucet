...
function argDegeri($ad) {global $argv;
foreach ($argv as $arg) {
if (strpos($arg, $ad . '=') === 0) {
...
global $c, $pidFile;$pid = calisanPid($pidFile);
if (!$pid) {
...
if (strpos($submit['body'], 'OK|') === false) {echo $c['merah'] . "  [!] API gönderimi başarısız: " . $submit['body'] . "\n" . $c['reset'];
...
// =============================
// TOKEN İÇİN ESNEK ARAMA FONKSİYONU
// =============================
function getToken($html) {
$token_patterns = ['session-token', '_token', 'csrf_token', 'token', 'csrf'];
foreach ($token_patterns as $p) {
...
preg_match('/data-sitekey="([^"]+)"/i', $html, $m_sitekey);$sitekey = $m_sitekey[1] ?? 'b56ad4c0-05d6-4218-b604-a54c67a8cede';

if (empty($m_email_name[1])) {
echo $c['merah']."  [-] E-posta formu bulunamadı. Atlanıyor...\n".$c['reset'];
sleep(1); continue;
}
...
$req2 = skibidixxx($post_url ?? $target_url, "POST", $payload, $headers, $cookieFile);
...
echo $c['merah']."  [!] Cloudflare engeli! JavaScript veya cookie clearance gerekli.\n".$c['reset'];
}
sleep(1); continue;
}
...
// SADECE GERÇEK HATA KUTULARINDA LİMİT KONTROLÜ YAP
if (preg_match('/<div class="alert alert-danger[^>]*>(.*?)<\/div>/is', $responHTML, $msg)) {
$pesan = trim(preg_replace('/\s+/', ' ', strip_tags(str_replace('&times;', '', $msg[1]))));
$pesan_lower = strtolower($pesan);
if (strpos($pesan_lower, 'limit') !== false ||
strpos($pesan_lower, 'sufficient') !== false ||
