...
APP_ORIGIN = "https://app.theopenearn.com"
...
def banner():
    ...
    print_box_row("", M)
    print(box_line())
    print_box_row("APP: NİNOCOIN • SOURCE: TheOpenEarn • KEY: OFF • VER: 1.1.7", G)
    print(box_line("╚", "═", "╝"))
...
class OpenEarnEngine:
    def __init__(self, query_id, sess_name, refresh_init_data=None):
        self.session = None
        self.auth_query_id = query_id
        self.refresh_init_data = refresh_init_data
        self.user_agent_index = random.randrange(len(USER_AGENTS))
        self.headers = {}
        self.username = sess_name
        self.user_id = "0"
        self.failed = False
        self.configure_headers(query_id)
        self.reset_session()
        try:
    ...
    def configure_headers(self, query_id):
        self.auth_query_id = query_id
        self.headers = {
            ...
            "User-Agent": USER_AGENTS[self.user_agent_index],
    ...
    def rotate_transport(self, reason):
        self.user_agent_index = (self.user_agent_index + 1) % len(USER_AGENTS)
        self.configure_headers(self.auth_query_id)
        self.reset_session()
        self.update_status("Gateway Fix", 94, detail=f"{reason}: new HTTP session + browser headers applied")

    async def refresh_authorization(self, reason):
        if not self.refresh_init_data:
            return False
        self.update_status("Auth Refresh", 94, detail=f"{reason}: refreshing Telegram webview token")
        try:
            init_data = await asyncio.wait_for(self.refresh_init_data(), timeout=INIT_TIMEOUT)
        except Exception as exc:
            self.update_status("Auth Wait", 94, detail=f"Token refresh failed, will retry later: {short_payload({'error': str(exc)})}")
            return False
        if not init_data:
            self.update_status("Auth Wait", 94, detail="Telegram returned empty webview token; will retry later")
            return False
        self.configure_headers(init_data)
        return True
    ...
    async def wait_for_server(self, label, status_code, payload, cycle):
        ...
        if status_code in {NETWORK_ERROR_STATUS, 502, 503, 504, 520, 522, 524}:
            self.rotate_transport(f"{label}: {issue}")
        if status_code == 502 or cycle % 3 == 0:
            await self.refresh_authorization(f"{label}: {issue}")
...
async def start_account(sess_path, sess_name, mode):
    ...
    bot = OpenEarnEngine(init_data, sess_name, refresh_init_data=lambda: get_init_data(sess_path))
